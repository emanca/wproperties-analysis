...
Hdata_obs_pos = Hdata_obs[...,1,:]
Hdata_obs_neg = Hdata_obs[...,0,:]

pos = np.sum(Hdata_obs_pos.values(),axis=(0,1,-1)).ravel()
pos_var = np.sum(Hdata_obs_pos.variances(),axis=(0,1,-1)).ravel()
neg = np.sum(Hdata_obs_neg.values(),axis=(0,1,-1)).ravel()
neg_var = np.sum(Hdata_obs_neg.variances(),axis=(0,1,-1)).ravel()

print(pos)
print(neg)

#sumw = np.concatenate((H[sum,sum,:,:,0,sum].values().ravel(),H[sum,sum,:,:,1,sum].values().ravel()))#HERE
sumw = np.concatenate((neg,pos))#HERE

#sumw = H[sum,sum,:,:,1,sum].values().ravel()
#sumw = Hdata_obs[...,1].values().ravel() #picking the positive charge
sumw2 = np.concatenate((neg_var,pos_var)) #two charges#HERE
...
threshold_y = np.digitize(2.0,yBins)-1
...
maskedchans = ['Wlike_plus','Wlike_minus']
...
# import mplhep as hep
# print(data_obs.shape,Hdata_obs_new[...,0].shape,data_obs.dtype)
# print(np.stack(df.query("charge==-1.")['nominal'].values,axis=-1).shape)
# t = np.sum(np.concatenate((np.stack(df.query("charge==-1.")['nominal'].values,axis=-1),np.stack(df.query("charge==1")['nominal'].values,axis=-1)),axis=0),axis=-1)

# print(np.sum(Hdata_obs_new[...]),np.sum(t))
# hep.histplot(data_obs/t)
# plt.show()
'''$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'''
