...
class plotter:
    ...
    def plotStack(self,skipSyst=[]):
        ...
        for var,varInfo in list(self.variableDict.items()) :
            ...
            hRmp = self.histoDict['WToMu'+s+var].Clone('Rmp'+var)
            hRmp.Divide(self.histoDict['WToMu'+'plus'+var])
            # hRmp.Scale(0.)
            print("-----------DEBUG-----------", var)
            try : 
                print('NOMINAL',hRmp.GetBinContent(5),hRmp.GetBinContent(10),hRmp.GetBinContent(15),hRmp.GetBinContent(40) )
            except :
                "worng bin"

            hData = self.histoDict['Data'+s+var].Clone('hData_'+var)
            self.histoDict['Data'+'plus'+var].Multiply(hRmp)
            hData.Add(self.histoDict['Data'+'plus'+var],-1)
            self.histoDict['Data'+s+var] = hData
            ...
            for sample in self.sampleOrder :
                ...
                self.histoDict[sample+'plus'+var].Multiply(hRmp)
                self.histoDict[sample+s+var].Add(self.histoDict[sample+'plus'+var],-1)
            ...
            for sKind, sList in list(bkg_utils.bkg_systematics.items()):
                ...
                for sName in sList :
                    ...
                    hRmp_var = self.histoDict['WToMu'+s+var+sName].Clone('Rmp'+var)
                    hRmp_var.Divide(self.histoDict['WToMu'+'plus'+var+sName])
                    # hRmp_var.Scale(0.)
                    try : 
                        print(var,hRmp_var.GetBinContent(5),hRmp_var.GetBinContent(10),hRmp_var.GetBinContent(15),hRmp_var.GetBinContent(40) )
                    except :
                        "worng bin"
                    
                    for sample in self.sampleOrder :
                        self.histoDict[sample+'plus'+var+sName].Multiply(hRmp_var)
                        # self.histoDict[sample+'plus'+var+sName].Multiply(hRmp)
                        self.histoDict[sample+s+var+sName].Add(self.histoDict[sample+'plus'+var+sName],-1)
        ...
        for var,varInfo in list(self.variableDict.items()) :
            ...
            for sKind, sList in list(self.extSyst.items()): #summing MCs
                for sName in sList :    
                    ...
                    # hRmp_var = self.histoDict['WToMu'+s+var].Clone('Rmp'+var)
                    # hRmp_var.Divide(self.histoDict['WToMu'+'plus'+var])
